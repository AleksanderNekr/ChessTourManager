...
public abstract class TournamentBase : INameable
{
	...
	public IReadOnlySet<Player> PlayersWithoutGroup => _playersWithoutGroup ??= new HashSet<Player>(new INameable.ByNameEqualityComparer<Player>());
	...
	{
		var isUnique = _groups.Add(new Group(groupId, groupName));
	...
	internal RemoveGroupResult TryRemoveGroup(Id<Guid> groupId)
	{
		...
		var removed = _groups.Remove(group);
